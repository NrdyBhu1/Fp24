...
class Bar:
    ...
    def render(self, win):
        ...
        self.bar.centerx = self.holder.x + self.value * (self.holder.w - self.holder.x)
    ...
    def dragged(self, event):
        ...
        if self.bar.x < self.holder.x:
            self.bar.x = self.holder.x-20
        elif self.bar.x > self.holder.x+self.holder.w:
            self.bar.x = self.holder.x+self.holder.w-20
        ...
        self.value = (round((self.bar.centerx-self.holder.x)*100/(self.holder.w-self.holder.x)/10)/10)
